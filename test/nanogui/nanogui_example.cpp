...
public:
    ...
    virtual void drawContents() {
        ...
        Matrix4f model;
        model.setIdentity();
        model.topLeftCorner<3, 3>() = Matrix3f(Eigen::AngleAxisf(0.6f * M_PI, Vector3f::UnitZ()));

        Matrix4f vp;
        vp.setIdentity();
        vp.topLeftCorner<3,3>() = Matrix3f(Eigen::AngleAxisf((float) glfwGetTime(),  Vector3f::UnitZ())) * 0.25f;
...
private:
    ...
    // using imagesDataType = vector<pair<xture, GLTexture::handleType>>;
